	...
	{((props.totalPages === "-1" || props.totalPages > 1)) ? (<>
		...
		<div className="center page buttons">
			<div className="ui borderless pagination menu">
				{("$showFirstLast") ? (<>
				<a className={`${(props.isFirst) ? `disabled` : ""} item navigation`} {(!(props.isFirst)) ? (<>href={`${("$paginationParams") ? `?` : ""}`}</>) : null}>
				...
				<a className={`${(!(props.hasPrevious)) ? `disabled` : ""} item navigation`} {(props.hasPrevious) ? (<>href={`?page=${String(props.previous ?? "")}${("$paginationParams") ? `&` : ""}`}</>) : null}>
				...
				{(((undefined /* $pages */)) as any[] ?? []).map((item: any, _i: number) => (<React.Fragment key={_i}>
					{(item.num === "-1") ? (<>
						...
						<a className={`${((props.isCurrent && "$pagesLen" > 1)) ? `active ` : ""}item`} {(!(item.isCurrent)) ? (<>href={`?page=${String(props.num ?? "")}${("$paginationParams") ? `&` : ""}`}</>) : null}>{item.num as any}</a>
					</>)}
				</React.Fragment>))}
				<a className={`${(!(props.hasNext)) ? `disabled` : ""} item navigation`} {(props.hasNext) ? (<>href={`?page=${String(props.next ?? "")}${("$paginationParams") ? `&` : ""}`}</>) : null}>
					<span className="navigation_label">{i18n("repo.issues.next")}</span>
					<span className="svg-icon" aria-label="octicon-chevron-right"></span>
				</a>

				{("$showFirstLast") ? (<>
				<a className={`${(props.isLast) ? `disabled` : ""} item navigation`} {(!(props.isLast)) ? (<>href={`?page=${String(props.totalPages ?? "")}${("$paginationParams") ? `&` : ""}`}</>) : null}>
