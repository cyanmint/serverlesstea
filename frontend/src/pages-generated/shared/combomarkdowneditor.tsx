...
{/* $mentionsLink */}
{/* $supportEasyMDE */}
<div {(props.containerId) ? (<>id={String(props.containerId ?? "")}</>) : null} className={`combo-markdown-editor ${(props.customInit) ? `custom-init` : ""} ${String(props.containerClasses ?? "")}`}
		...
		{("$mentionsLink") ? (<>data-mentions-url={String("" ?? "")}</>) : null}
>
	{("$previewMode" !== "none") ? (<>
	...
	<div className="ui tab active" data-tab-panel="markdown-writer">
		...
		<text-expander keys=": @ #" multiword="#" suffix="">
			<textarea className="markdown-text-editor"
				{(props.textareaName) ? (<>name={String(props.textareaName ?? "")}</>) : null} {(props.textareaMaxLength) ? (<>maxlength={String(props.textareaMaxLength ?? "")}</>) : null}
				{(props.textareaPlaceholder) ? (<>placeholder={String(props.textareaPlaceholder ?? "")}</>) : null} {("$ariaLabel") ? (<>aria-label={String("" ?? "")}</>) : null}
				{(props.disableAutosize) ? (<>data-disable-autosize={String(props.disableAutosize ?? "")}</>) : null}
