...
<div className="ui container fluid padded projects-view" data-global-init="initRepoProjectsView">
	...
	<div id="project-board" className={`board ${("$canWriteProject") ? `sortable` : ""}`} data-project-board-writable={String("" ?? "")} {("$canWriteProject") ? (<>data-url={`${String(props.link ?? "")}/move`}</>) : null}>
		{((props.columns) as any[] ?? []).map((item: any, _i: number) => (<React.Fragment key={_i}>
			<div className="project-column" {(item.color) ? (<>style={`background: ${String(props.color ?? "")} !important; color:  !important`}</>) : null} data-id={String(props.iD ?? "")} data-sorting={String(props.sorting ?? "")} data-url={`${String(props.link ?? "")}/${String(props.iD ?? "")}`}>
				<div className={`project-column-header${("$canWriteProject") ? ` tw-cursor-grab` : ""}`}>
					...
					<div className="project-column-title-text flex-text-inline gt-ellipsis" {(item.default) ? (<>data-tooltip-content={String(i18n("repo.projects.column.default_column_hint") ?? "")}</>) : null}>
				...
				<div className="divider"{(item.color) ? (<> style={`color:  !important`}</>) : null}></div>
