...
export default function RubygemsDependencies(props: Record<string, unknown>) {
  return (<>
{(props.dependencies) ? (<>
...
<table className="ui single line very basic table">
	...
	<tbody>
		...
		<tr>
			...
			<td>{((item.version) as any[] ?? []).map((item: any, _i: number) => (<React.Fragment key={_i}>{(props.i > 0) ? (<>, </>) : null}{/* TODO: {{$v.Restriction}} */}{/* TODO: {{$v.Version}} */}</React.Fragment>))}</td>
		</tr>
		</React.Fragment>))}
	</tbody>
</table>
