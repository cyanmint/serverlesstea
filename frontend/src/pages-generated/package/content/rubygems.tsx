...
{(props.packageDescriptor?.package?.type === "rubygems") ? (<>
	...
	{((props.packageDescriptor?.metadata?.requiredRubyVersion || props.packageDescriptor?.metadata?.requiredRubygemsVersion)) ? (<>
		...
		<div className="ui attached segment">
			{(props.packageDescriptor?.metadata?.requiredRubyVersion) ? (<><p>{i18n("packages.rubygems.required.ruby")}: {((props.packageDescriptor?.metadata?.requiredRubyVersion) as any[] ?? []).map((item: any, _i: number) => (<React.Fragment key={_i}>{(props.i > 0) ? (<>, </>) : null}{/* TODO: {{$v.Restriction}} */}{/* TODO: {{$v.Version}} */}</React.Fragment>))}</p></>) : null}
			{(props.packageDescriptor?.metadata?.requiredRubygemsVersion) ? (<><p>{i18n("packages.rubygems.required.rubygems")}: {((props.packageDescriptor?.metadata?.requiredRubygemsVersion) as any[] ?? []).map((item: any, _i: number) => (<React.Fragment key={_i}>{(props.i > 0) ? (<>, </>) : null}{/* TODO: {{$v.Restriction}} */}{/* TODO: {{$v.Version}} */}</React.Fragment>))}</p></>) : null}
		</div>
	</>) : null}
