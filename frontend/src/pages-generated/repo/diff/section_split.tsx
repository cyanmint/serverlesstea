		...
		{((item.getType !== 2 || !(hasmatch))) ? (<>
			<tr className={`${String(props.getHTMLDiffLineType ?? "")}-code nl- ol-`} data-line-type={String(props.getHTMLDiffLineType ?? "")}>
				{(item.getType === 4) ? (<>
					...
					{/* $match */}
					{/* TODO: {{- $leftDiff := ""}} */}{(item.line?.leftIdx) ? (<>{/* TODO: {{$leftDiff = $section.GetComputedInlineDiffFor $line ctx.Locale}} */}</>) : null}
					{/* TODO: {{- $rightDiff := ""}} */}{(item.match?.rightIdx) ? (<>{/* TODO: {{$rightDiff = $section.GetComputedInlineDiffFor $match ctx.Locale}} */}</>) : null}
