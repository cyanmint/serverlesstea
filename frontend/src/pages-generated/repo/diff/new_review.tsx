...
export default function NewReview(props: Record<string, unknown>) {
  return (<>
<div id="review-box" {(props.repository?.isArchived) ? (<>data-tooltip-content={String(i18n("repo.archive.pull.nocomment") ?? "")}</>) : null}>
	<button className={`ui tiny primary button tw-pr-1 js-btn-review ${(!(props.isShowingAllCommits)) ? `disabled` : ""}`}
		{(!(props.isShowingAllCommits)) ? (<>data-tooltip-content={String(i18n("repo.pulls.review_only_possible_for_full_diff") ?? "")}</>) : null}
