...
export default function Github(props: Record<string, unknown>) {
  return (<>
		...
		<div className="ui attached segment">
			...
			<form className="ui form left-right-form" action={String(props.link ?? "")} method="post" onSubmit={(props.onSubmit as any) ?? ((e: React.FormEvent) => e.preventDefault())}>
				...
				<div className={`inline required field ${(props.err_CloneAddr) ? `error` : ""}`}>
					...
					<span className="help">
					{i18n("repo.migrate.clone_address_desc")}
					</span>
				</div>
				...
				<div className={`inline field ${(props.err_Auth) ? `error` : ""}`}>
					...
					<a target="_blank" href="https://docs.github.com/en/github/authenticating-to-github/creating-a-personal-access-token"><span className="svg-icon" aria-label="octicon-question"></span></a>
					<span className="help">
					{i18n("repo.migrate.github_token_desc")}
					</span>
				</div>
