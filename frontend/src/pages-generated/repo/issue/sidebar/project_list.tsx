...
<div className="issue-sidebar-combo sidebar-project-combo" data-selection-mode="multiple" data-update-algo="all"
		{("$pageMeta.Issue") ? (<>data-update-url={`/issues/projects?issue_ids=`}</>) : null}
>
	...
	<div className={`ui dropdown full-width ${(!("$pageMeta.CanModifyIssueOrPull")) ? `disabled` : ""}`}>
		<a className="fixed-text muted">
			<strong>{i18n("repo.issues.new.projects")}</strong> {("$pageMeta.CanModifyIssueOrPull") ? (<><span className="svg-icon" aria-label="octicon-gear"></span></>) : null}
		</a>
		<div className="menu">
			{(("$data.OpenProjects" || "$data.ClosedProjects")) ? (<>
			...
			<div className="scrolling menu flex-items-menu">
				...
				{(("$data.OpenProjects" && "$data.ClosedProjects")) ? (<><div className="divider"></div></>) : null}
				{("$data.ClosedProjects") ? (<>
	...
	<div className="ui list tw-my-2 flex-relaxed-list issue-sidebar-project-cards" data-combo-list-inited="true">
		<div className={`item empty-list ${("$data.ProjectCards") ? `tw-hidden` : ""}`}>{i18n("repo.issues.new.no_projects")}</div>
	{(($data.ProjectCards) as any[] ?? []).map((item: any, _i: number) => (<React.Fragment key={_i}>
		...
		<div className={`item ${("$selectedColumn") ? `sidebar-project-card` : ""}`}>
			...
			{(("$selectedColumn" && "$pageMeta.CanModifyIssueOrPull")) ? (<>
					...
					<div className="ui dropdown full-width">
						<div className="flex-text-block tw-ml-[16px]">{/* align with the "project" icon */}
							<div className="interact-bg tw-px-2 tw-py-1 tw-rounded flex-text-block fixed-text">
								{("$selectedColumn") ? (<>
									{("$card.SelectedColumn.Color") ? (<><span className="color-icon icon-size-8" style={`background-color: `}></span></>) : null}
						...
						<div className="menu flex-items-menu">
							{(($card.Columns) as any[] ?? []).map((item: any, _i: number) => (<React.Fragment key={_i}>
								<a className="item" data-value={String("" ?? "")}>
									<span className="item-check-mark"><span className="svg-icon" aria-label="octicon-check"></span></span>
									{("$columnItem.Color") ? (<><span className="color-icon icon-size-8" style={`background-color: `}></span></>) : null}
				...
				<div className="flex-text-block tw-my-1 tw-ml-[22px]">{/* align with the "project" icon */}
					{("$selectedColumn.Color") ? (<><span className="color-icon icon-size-8" style={`background-color: `}></span></>) : null}
