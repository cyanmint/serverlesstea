...
{/* $pageMeta */}
{/* $data */}
{/* $issueMilestone */}{(("$pageMeta.Issue" && "$pageMeta.Issue.Milestone")) ? (<>{/* TODO: {{$issueMilestone = $pageMeta.Issue.Milestone}} */}</>) : null}
<div className="divider"></div>
<div className="issue-sidebar-combo" data-selection-mode="single" data-update-algo="all"
		{("$pageMeta.Issue") ? (<>data-update-url={`/issues/milestone?issue_ids=`}</>) : null}
>
	...
	<div className={`ui dropdown full-width ${(!("$pageMeta.CanModifyIssueOrPull")) ? `disabled` : ""}`}>
		<a className="fixed-text muted">
			<strong>{i18n("repo.issues.new.milestone")}</strong> {("$pageMeta.CanModifyIssueOrPull") ? (<><span className="svg-icon" aria-label="octicon-gear"></span></>) : null}
		</a>
		<div className="menu">
			{((!("$data.OpenMilestones") && !("$data.ClosedMilestones"))) ? (<>
				...
				<div className="scrolling menu flex-items-menu">
					...
					{(("$data.OpenMilestones" && "$data.ClosedMilestones")) ? (<><div className="divider"></div></>) : null}
					{("$data.ClosedMilestones") ? (<>
