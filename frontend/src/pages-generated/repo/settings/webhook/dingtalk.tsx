...
export default function Dingtalk(props: Record<string, unknown>) {
  return (<>
{(props.hookType === "dingtalk") ? (<>
	<p>{i18n("repo.settings.add_web_hook_desc")}</p>
	<form className="ui form" action={`${String(props.baseLink ?? "")}/dingtalk/`} method="post" onSubmit={(props.onSubmit as any) ?? ((e: React.FormEvent) => e.preventDefault())}>
